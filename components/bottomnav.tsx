...
export default function BottomNav({ isAuth, handleLogout }: BottomNavProps) {
  ...
  // Ambil warna slide dengan aman (Fallback jika index tidak ditemukan)
  const currentSlideColor = links[currentSlideIndex]?.color || "#EAB308";
    ...
    <div className="fixed bottom-3 left-0 right-0 z-[100000] flex justify-center px-3 pointer-events-none pb-[env(safe-area-inset-bottom)]">
      <nav
        ...
        style={{
          ...
          borderColor: isHome
            ? `color-mix(in srgb, ${currentSlideColor} 30%, transparent)`
