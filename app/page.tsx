...
export default function Home() {
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    const payload = { username, pesan: input.trim(), device_id: localStorage.getItem('device_id') || 'guest', user_browser: getBrowserInfo(), is_private: isPrivate, private_with: isPrivate ? (activeTab === 'user' ? 'admin' : selectedPrivateUser) : null };
    const { error } = await supabase.from('messages').insert([payload]);
    if (error) alert("Gagal: " + error.message);
    else { setInput(''); setLastSent(Date.now()); await fetchData(); }
    setSending(false);
  };

  const editMsg = async (id: number) => { const newText = prompt("Edit pesan:", messages.find(m => m.id === id)?.pesan || ""); if (newText) { await supabase.from('messages').update({ pesan: newText }).eq('id', id); fetchData(); } };
  const editNama = async (id: number) => { const m = messages.find(m => m.id === id); if (!m) return; const newName = prompt("Ubah nama user:", m.username); if (newName) { await supabase.from('messages').update({ username: newName }).eq('device_id', m.device_id); fetchData(); } };
  const deleteMsg = async (id: number) => { if (confirm("Hapus pesan?")) { await supabase.from('messages').delete().eq('id', id); fetchData(); } };
  const blockUser = async (device_id: string, username: string) => { if (confirm(`Blokir ${username}?`)) { await supabase.from('blocked_users').insert([{ device_id, username }]); fetchData(); } };
  const unblock = async (id: string) => { await supabase.from('blocked_users').delete().eq('device_id', id); fetchData(); };
  const inviteToPrivate = (device_id: string, username: string) => { if (confirm(`Ajak ${username} ke private chat?`)) { setChatMode('private'); setSelectedPrivateUser(device_id); } };
  ...
  if (!isAuth) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-gradient-to-br from-emerald-500 to-blue-600 text-white p-6">
        ...
        {activeTab === 'user' ? (
          <input className="w-full max-w-sm p-3 rounded text-black mb-3" placeholder="Nama Anda" value={username} onChange={(e) => setUsername(e.target.value.substring(0, 20))} maxLength={20} />
        ) : (
        ...
        <button onClick={() => activeTab === 'admin' ? (async() => { const { error } = await supabase.auth.signInWithPassword({ email: adminEmail, password: adminPass }); if (!error) { setIsAuth(true); setActiveTab('admin'); setUsername('Admin●ipix.my.id'); } else alert("Login Gagal");})() : (setIsAuth(true) || localStorage.setItem('saved_username', username))} className="bg-white text-emerald-600 px-8 py-3 rounded-full font-bold">Masuk Chat</button>
      </div>
    );
  }
      ...
      <div className="sticky top-0 z-10 p-3 bg-white/30 backdrop-blur-md border-b border-white/20">
        ...
        <div className="flex mt-3 bg-gray-100 rounded-full p-1 shadow">
          ...
          <button onClick={() => { setChatMode('private'); setSelectedPrivateUser(null); }} className={`relative flex-1 py-2.5 text-sm font-medium rounded-full transition-all ${chatMode === 'private' ? 'bg-emerald-600 text-white shadow' : 'text-gray-700 hover:bg-gray-200'}`}>
            {chatMode === 'private' ? '🔒 Private ' : ''} Chat dengan Admin {privateNotifCount > 0 && <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-bold min-w-5 h-5 px-1 flex items-center justify-center rounded-full animate-bounce">{formatNotif(privateNotifCount)}</span>}
          </button>
        </div>
      </div>
              ...
              <div key={m.id} className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm w-full">
                <div className="flex justify-between items-center mb-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] px-1 bg-gray-100 text-gray-500 rounded">{formatMessageTime(m.created_at)}</span>
                  </div>
                </div>
                ...
                {activeTab === 'admin' && (
                  <div className="flex gap-4 mt-2 text-[10px] flex-wrap">
                    ...
                    <button onClick={() => blockUser(m.device_id, m.username)} className="text-gray-400 font-bold underline">Blok</button>
