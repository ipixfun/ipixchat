...
const DEFAULT_APP_INFO = {
  appName: "ipixchat", version: "v1.0", apkSize: "4.45 MB",
  videoUrl: "https://res.cloudinary.com/bjamo8ld/video/upload/v1785508218/ipixchat_dryqj3.mp4",
  apkDownloadUrl: "https://ipix.my.id/ipixchat.apk", year: new Date().getFullYear(),
  featuresTitle: "Fitur aplikasi", videoSectionTitle: "Panduan Video",
  footerText: "IPIXCHAT", footerSubText: "ALL RIGHTS RESERVED", footerUrl: "https://ipix.my.id",
};

const DEFAULT_BANNER_INFO = {
  apkThankYouTitle: "Support App", apkThankYouDesc: "Terima kasih supportnya sudah menggunakan apk ipixchat. Nikmati pengalaman berinteraksi yang lebih cepat dan lancar.",
  webBadge: "Terima Kasih", webTitle: "Unduh aplikasi ipixchat", webDesc: "Dapatkan pengalaman interaksi yang lebih optimal, ringan, dan cepat langsung melalui perangkat Android Anda."
};

const DEFAULT_PLATFORM_INFO = {
  badge: "Platform Perpesanan", title: "Ruang Interaksi Personal bersama pix",
...
const DEFAULT_FEATURES = [
  { id: 1, title: "Akses Instan & Praktis", text: "Cukup daftarkan Username dan PIN 6 digit angka untuk langsung berinteraksi dengan saya/pix. Akses auto-login membuat sesi percakapan Anda berjalan mulus tanpa hambatan.", action: { label: "Mulai Chat", href: "/chat" }, isHighlight: false },
  { id: 2, title: "Pengiriman Media Interaktif", text: "Fitur berbagi gambar dan media yang responsif langsung dari kolom percakapan.", action: null, isHighlight: false },
  { id: 3, title: "Kustomisasi Tema Personal", text: "Sesuaikan antarmuka visual aplikasi sesuai selera Anda setelah berhasil masuk.", action: { label: "Ubah Tema", href: "/tema" }, isHighlight: false },
  { id: 4, title: "Navigasi & Ekosistem Terpadu", text: "Jelajahi informasi media sosial dan tautan ekosistem ipix hanya dengan menggeser atau melepas kontrol pill.", action: null, isHighlight: false },
  { id: 5, title: "Pengembangan Berkelanjutan", text: "Pembaruan fitur baru dan peningkatan performa akan terus dikembangkan secara rutin.", action: null, isHighlight: true }
];
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    if (isApkMode) {
      setShowVideo(false);
      if (!localStorage.getItem('ipix_apk_first_open')) {
    ...
    if (storedUser) setUserName(storedUser.split('●')[0]);
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      setShowBanner(!loggedIn && !isApkMode);
    });
    ...
    const fetchConfig = async () => {
      try {
        const { data, error } = await supabase.from('homepage_config').select('*').eq('id', 1).single();
        if (!error && data) {
          memoryCache = data;
  ...
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.auth.signInWithPassword({
        email: adminEmailInput, password: adminPasswordInput,
      });
      if (error) throw error;
      if (data.session) {
        setIsAdmin(true); setShowAdminLoginModal(false);
        setAdminEmailInput(''); setAdminPasswordInput(''); setLoginError('');
      }
  ...
  const toggleEditMode = () => {
    ...
    if (newMode) { setShowBanner(true); setShowVideo(true); setShowPlatform(true); setShowFeatures(true); }
  };
  ...
  const handleSaveAllChanges = async () => {
    try {
      ...
      const payload = {
        id: 1, app_info: appInfo, banner_info: bannerInfo, platform_info: platformInfo,
        features: features, ecosystem_links: ecosystemLinks, updated_at: new Date().toISOString()
      };
      memoryCache = payload;
      if (typeof window !== 'undefined') localStorage.setItem('ipix_homepage_config', JSON.stringify(payload));
      const { error } = await supabase.from('homepage_config').upsert(payload);
      if (error) throw error;
  ...
  const renderHighlightedText = (text: string) => {
    if (!text) return null;
    return text.split(/\b(pix)\b/i).map((part, index) => part.toLowerCase() === 'pix' ? (
      <span key={index} className="font-black uppercase tracking-wider drop-shadow-md brightness-[1.4] px-0.5" style={{ color: 'var(--accent)', filter: 'drop-shadow(0 0 12px color-mix(in srgb, var(--accent) 90%, white))' }}>PIX</span>
    ) : part);
  };
  ...
  return (
    <div suppressHydrationWarning className="w-full max-w-2xl mx-auto h-dvh flex flex-col pb-[70px] relative overflow-hidden bg-[var(--background)] font-sans text-xs">
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        @keyframes wave-loop-1 { 0% { transform: translate3d(0, 0, 0); } 100% { transform: translate3d(-50%, 0, 0); } }
        @keyframes wave-loop-2 { 0% { transform: translate3d(-50%, 0, 0); } 100% { transform: translate3d(0, 0, 0); } }
        .animate-wave-1 { animation: wave-loop-1 12s linear infinite; will-change: transform; }
        .animate-wave-2 { animation: wave-loop-2 8s linear infinite; will-change: transform; }
        .admin-input { width: 100%; background: color-mix(in srgb, var(--background) 50%, #000000); border: 1px solid color-mix(in srgb, var(--accent) 50%, transparent); color: white; padding: 8px 12px; border-radius: 10px; font-size: 12px; outline: none; transition: all 0.2s; }
        .admin-input:focus { border-color: var(--accent); box-shadow: 0 0 8px color-mix(in srgb, var(--accent) 40%, transparent); }
        .admin-label { display: block; font-size: 10px; font-weight: 800; color: var(--accent); margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.5px; }
      ` }} />
      ...
      <header className="sticky top-0 z-20 px-3.5 py-2.5 glass-card border-b" style={{ backgroundColor: "color-mix(in srgb, var(--background) 75%, transparent)", borderColor: "var(--card-border)" }}>
        <div className="flex items-center justify-between gap-1.5">
          ...
          <div className="flex items-center gap-1 justify-end">
            ...
            {isAdmin && (
              <button onClick={toggleEditMode} className="px-2.5 py-0.5 rounded-full text-[9px] font-extrabold tracking-wide bg-amber-500 text-black shadow-md active:scale-95">{isEditMode ? 'Tutup' : 'Edit'}</button>
            )}
          </div>
        </div>
      </header>
      ...
      <div className="flex-1 overflow-y-auto hide-scroll space-y-3 pt-2.5 pb-5 px-3">
            ...
            <motion.div key="banner-apk" initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} transition={{ duration: 0.2 }} className="p-3.5 sm:p-4 rounded-3xl relative overflow-hidden border text-center flex flex-col items-center justify-center gap-1 z-0" style={{ background: `linear-gradient(135deg, color-mix(in srgb, var(--accent) 22%, var(--background)) 0%, var(--card-bg) 100%)`, borderColor: "var(--card-border)" }}>
              <div onClick={() => setShowBanner(!showBanner)} className="w-full flex items-center justify-between gap-2 cursor-pointer group z-10">
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  {displayUserName && (
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wide shadow-md" style={{ backgroundColor: 'var(--accent)', color: 'var(--background)', boxShadow: '0 0 12px color-mix(in srgb, var(--accent) 50%, transparent)' }}>{displayUserName}</span>
                  )}
        ...
        {isEditMode ? (
          ...
          <div className="p-3.5 rounded-3xl text-left border" style={{ backgroundColor: "var(--card-bg)", borderColor: "var(--card-border)" }}>
            <div onClick={() => setShowPlatform(!showPlatform)} className="flex items-center justify-between gap-2 cursor-pointer group">
              ...
              <button aria-label="Toggle detail platform" className="w-6 h-6 rounded-full flex items-center justify-center shrink-0" style={{ backgroundColor: 'color-mix(in srgb, var(--accent) 10%, transparent)' }}>
        ...
        <div className="relative w-full rounded-3xl p-3.5 mt-3 border" style={{ backgroundColor: "var(--card-bg)", borderColor: "var(--card-border)", boxShadow: "0 8px 24px -8px rgba(0,0,0,0.2)" }}>
          ...
          <div onClick={() => !isEditMode && setShowVideo(!showVideo)} className="flex items-center justify-between gap-2 cursor-pointer group">
            ...
            {!isEditMode && (
              <button aria-label="Toggle panduan video" className="w-6 h-6 rounded-full flex items-center justify-center shrink-0" style={{ backgroundColor: 'color-mix(in srgb, var(--accent) 10%, transparent)' }}>
