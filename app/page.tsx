    ...
    <div className="flex flex-col h-screen bg-gray-100">
            ...
            <div className="flex justify-between items-start mb-2">
              ...
              {isAdmin && (
                <div className="flex gap-2">
                  <button onClick={async () => { await supabase.from('blocked_users').insert([{ device_id: msg.device_id }]); alert("Terblokir!"); }} className="text-[10px] bg-orange-100 text-orange-600 px-2 py-1 rounded font-bold">BLOCK</button>
                  <button onClick={async () => await supabase.from('messages').delete().eq('id', msg.id)} className="text-[10px] bg-red-100 text-red-600 px-2 py-1 rounded font-bold">HAPUS</button>
                </div>
              )}
            </div>
      ...
      <div className="bg-white border-t p-2">
        ...
        <form onSubmit={handleSendMessage} className="flex gap-2">
          <input value={inputMessage} onChange={(e) => setInputMessage(e.target.value)} className="flex-1 p-3 border rounded text-black" placeholder="Pesan atau URL Gambar..." />
