...
export default function Home() {
  ...
  const scrollMsg = (id: number) => { const el = document.getElementById(`msg-${id}`); if (el) { el.scrollIntoView({ behavior: 'smooth', block: 'center' }); const bc = 'anim-bg-blink-cream'; el.classList.add(bc); setTimeout(() => el.classList.remove(bc), 1500); } };
    ...
    try {
      ...
      if (auth.isAuth) {
        const [{ count: pubC }, { count: privC }] = await Promise.all([
          ...
          supabase.from('messages').select('*', { count: 'exact', head: true }).eq('is_private', true).or(ui.tab === 'user' ? `device_id.eq.${currentDeviceId},private_with.eq.${currentDeviceId}` : `id.gt.0`)
        ]);
