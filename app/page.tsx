...
export default function HomePage() {
  ...
  // 1. Muat Data Tersimpan & Cek Status Admin
  useEffect(() => {
    ...
    const savedContent = localStorage.getItem('ipix_home_content');
    if (savedContent) {
      try {
        const parsed = JSON.parse(savedContent);
        if (parsed.appInfo) setAppInfo({ ...DEFAULT_APP_INFO, ...parsed.appInfo });
        if (parsed.bannerInfo) setBannerInfo({ ...DEFAULT_BANNER_INFO, ...parsed.bannerInfo });
        if (parsed.platformInfo) setPlatformInfo({ ...DEFAULT_PLATFORM_INFO, ...parsed.platformInfo });
  ...
  const toggleEditMode = () => {
    ...
    // Buka semua kolom otomatis jika masuk ke Mode Edit agar mudah dilihat
    if (newMode) {
      setShowVideo(true);
  ...
  const handleSaveAllChanges = () => {
    const dataToSave = { appInfo, bannerInfo, platformInfo, features };
    localStorage.setItem('ipix_home_content', JSON.stringify(dataToSave));
    setSaveSuccessMsg('Perubahan berhasil disimpan permanen!');
  ...
  return (
    <div className="w-full max-w-2xl mx-auto h-dvh flex flex-col pb-[70px] relative overflow-hidden bg-[var(--background)]">
      ...
      <AnimatePresence>
        {showAdminLoginModal && (
          <motion.div 
            ...
            className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 glass-card"
          >
            <motion.div 
        ...
        {showDownloadConfirm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 glass-card">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="w-full max-w-sm p-5 rounded-3xl text-center shadow-2xl" style={{ backgroundColor: "var(--card-bg)", border: "1px solid var(--card-border)" }}>
              <h3 className="text-lg font-black mb-2" style={{ color: "var(--foreground-heading)" }}>Konfirmasi Unduhan</h3>
      ...
      {/* BAR CONTROL ADMIN */}
      {isAdmin && (
        <div className="bg-amber-500 text-black px-4 py-2 flex items-center justify-between text-xs z-30 shadow-md">
          <div className="flex items-center gap-2 font-black">
            <span className="w-2.5 h-2.5 rounded-full bg-red-600 animate-pulse" />
            <span>MODE ADMIN</span>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={toggleEditMode} className="px-3 py-1.5 rounded-lg text-[10px] font-black bg-black text-white active:scale-95 transition-transform">
              {isEditMode ? 'TUTUP EDIT' : 'BUKA EDIT TEKS'}
            </button>
            {isEditMode && (
              <button onClick={handleSaveAllChanges} className="px-3 py-1.5 rounded-lg text-[10px] font-black bg-emerald-700 text-white active:scale-95 transition-transform">
                SIMPAN
              </button>
            )}
            <button onClick={handleAdminLogout} className="px-2 py-1.5 rounded-lg text-[10px] font-black bg-red-800 text-white">Keluar</button>
          </div>
        </div>
      )}
      ...
      {saveSuccessMsg && (
        <div className="fixed top-16 left-1/2 -translate-x-1/2 z-50 bg-emerald-500 text-white text-xs font-bold px-5 py-2.5 rounded-full shadow-2xl animate-bounce">
          {saveSuccessMsg}
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto hide-scroll space-y-4 sm:space-y-5 pt-3 pb-6 px-3.5 sm:px-6">
        ...
        {/* 2. Banner APK / Web (DENGAN MODE EDIT) */}
        {isEditMode ? (
          <div className="p-4 bg-amber-500/10 border border-amber-500/30 rounded-2xl space-y-3">
            ...
            <div className="space-y-2">
              ...
              <input type="text" className="admin-input" placeholder="Badge (Contoh: Aplikasi Android)" value={bannerInfo.webBadge} onChange={e => setBannerInfo({...bannerInfo, webBadge: e.target.value})} />
              <input type="text" className="admin-input" placeholder="Judul Banner Web" value={bannerInfo.webTitle} onChange={e => setBannerInfo({...bannerInfo, webTitle: e.target.value})} />
              <textarea rows={2} className="admin-input" placeholder="Deskripsi Banner Web" value={bannerInfo.webDesc} onChange={e => setBannerInfo({...bannerInfo, webDesc: e.target.value})} />
            </div>
            <div className="space-y-2 mt-4 border-t border-amber-500/30 pt-3">
              <label className="admin-label">Banner APK (Terima Kasih)</label>
              <input type="text" className="admin-input" placeholder="Judul Banner APK" value={bannerInfo.apkThankYouTitle} onChange={e => setBannerInfo({...bannerInfo, apkThankYouTitle: e.target.value})} />
              <textarea rows={2} className="admin-input" placeholder="Deskripsi Banner APK" value={bannerInfo.apkThankYouDesc} onChange={e => setBannerInfo({...bannerInfo, apkThankYouDesc: e.target.value})} />
            </div>
          </div>
        ) : (
          isApk ? (
            <motion.div className="py-8 px-4 rounded-2xl relative overflow-hidden border text-center flex flex-col items-center justify-center gap-2" style={{ background: `linear-gradient(135deg, color-mix(in srgb, var(--accent) 12%, var(--background)) 0%, var(--card-bg) 100%)`, borderColor: "var(--card-border)" }}>
              ...
              <div className="relative z-10 w-full">
                <h2 className="text-2xl font-black tracking-tight mb-1.5" style={{ color: "var(--accent)" }}>{bannerInfo.apkThankYouTitle}</h2>
            ...
            <motion.div className="p-4 sm:p-6 rounded-2xl relative overflow-hidden border" style={{ background: `linear-gradient(135deg, color-mix(in srgb, var(--accent) 25%, var(--background)) 0%, var(--card-bg) 100%)`, borderColor: "var(--card-border)" }}>
              <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3.5">
                <div className="space-y-1 flex-1">
                  ...
                  <h2 className="text-xl font-black tracking-tight" style={{ color: "var(--foreground-heading)" }}>
                    {bannerInfo.webTitle} <span className="text-xs font-bold opacity-80">({appInfo.apkSize})</span>
                  </h2>
          ...
          <motion.div className="p-4 rounded-2xl text-left border" style={{ backgroundColor: "var(--card-bg)", borderColor: "var(--card-border)" }}>
            <div onClick={() => setShowPlatform(!showPlatform)} className="flex items-center justify-between gap-2 cursor-pointer group">
              <div>
                ...
                <h2 className="text-lg font-black tracking-tight leading-snug" style={{ color: "var(--foreground-heading)" }}>{platformInfo.title}</h2>
              </div>
            ...
            <AnimatePresence initial={false}>
              {showPlatform && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                  <p className="text-xs opacity-85 leading-relaxed mt-3 pt-2 border-t" style={{ color: "var(--foreground)", borderColor: "var(--card-border)" }}>{platformInfo.description}</p>
                </motion.div>
              )}
            </AnimatePresence>
          </motion.div>
          ...
          <motion.div className="p-4 rounded-2xl border" style={{ backgroundColor: "var(--card-bg)", borderColor: "var(--card-border)" }}>
            <div onClick={() => setShowFeatures(!showFeatures)} className="flex items-center justify-between gap-2 border-b pb-3 cursor-pointer group" style={{ borderColor: "var(--card-border)" }}>
              <div className="flex items-center gap-2.5">
                ...
                <h3 className="text-base font-black tracking-tight" style={{ color: "var(--foreground-heading)" }}>Fitur Utama</h3>
              </div>
              <div className="flex items-center gap-2">
                    ...
                    {features.map((feature, idx) => (
                      <li key={feature.id} className="flex gap-2.5">
                        ...
                        <div>
                          ...
                          <p className={`text-[11px] opacity-85 leading-relaxed ${feature.isHighlight ? 'font-bold italic' : ''}`} style={{ color: feature.isHighlight ? "var(--accent)" : "var(--foreground)" }}>{feature.text}</p>
                          {feature.action && (
                            <div className="mt-2">
            ...
            {showVideo && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                <div className="relative w-full rounded-xl overflow-hidden bg-black flex justify-center mt-1">
                  <video src={appInfo.videoUrl} controls loop playsInline className="w-full h-auto max-h-[50vh] object-contain rounded-xl" />
                </div>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <div className="flex flex-col items-center justify-center mt-6 mb-2 space-y-1">
          ...
          <p onClick={() => !isAdmin && setShowAdminLoginModal(true)} className="text-[10px] font-black uppercase tracking-[0.2em] cursor-pointer hover:opacity-80" style={{ color: "var(--accent)" }}>
