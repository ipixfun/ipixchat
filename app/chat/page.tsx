...
export default function Home() {
  ...
  const [accountChangedByAdmin, setAccountChangedByAdmin] = useState(false);
  ...
  const applyCensor = (t: string) => {
    let r = t;
    censor.words.forEach((w) => { if (w.trim()) r = r.replace(new RegExp(`\\b${w}\\b`, "gi"), "***"); });
    return r;
  };
  const copyTxt = (t: string, l: string) => {
    navigator.clipboard.writeText(t);
    showAlert(`${l} berhasil disalin!`, "Berhasil", "success");
  };
  ...
  const handleLogout = async () => {
    ...
    setAccountChangedByAdmin(false);
  ...
  const fetchData = useCallback(async () => {
    ...
    const savedPin = typeof window !== "undefined" ? (localStorage.getItem("remembered_pin") || localStorage.getItem("saved_pin") || sessionStorage.getItem("saved_pin") || localStorage.getItem("user_pin") || localStorage.getItem("pin") || "") : "";
    ...
    try {
      if (targetUser !== "Admin●ipix.my.id") {
        ...
        if (profileCheck) {
          ...
          if (currentValidPin && currentValidPin.length === 6 && profileCheck.pin !== currentValidPin) {
            setAccountChangedByAdmin(true); return;
          }
        } else {
          setAccountChangedByAdmin(true); return;
        }
      ...
      if (isAdminTab) {
        ...
        if (profilesData) {
          const formattedPrivUsers = profilesData.filter((p: any) => !p.username?.toLowerCase().includes("admin")).map((p: any) => {
            const uName = p.username;
            const userMsgs = (allMessages || []).filter((m: any) => m.username === uName || m.chat_with === uName);
            ...
            const lastMsgObj = userMsgs[0];
            return { username: uName, pin: p.pin || "", umur: p.umur || "", berat: p.berat || "", totalUserMsgs, totalAdminMsgs, count: 0, last_message: lastMsgObj ? lastMsgObj.pesan : null, last_active: lastMsgObj ? lastMsgObj.created_at : p.created_at };
          });
  ...
  const dbActions = {
    editMsg: async (mOrId: any) => {
      ...
      setTimeout(() => {
        ...
        if (inputEl) { inputEl.focus(); inputEl.setSelectionRange(inputEl.value.length, inputEl.value.length); inputEl.scrollIntoView({ behavior: "smooth", block: "center" }); }
      }, 100);
    },
    ...
    deleteAudioOnly: async (m: any) => {
      ...
      setConfirmModal({
        isOpen: true, type: "danger", title: "Hapus Voice Note (Admin)", message: "Voice Note ini akan dihapus dari Cloudinary dan database. Lanjutkan?", confirmText: "Hapus VN", cancelText: "Batal",
        onConfirm: async () => {
          try {
            const res = await fetch("/api/delete-audio", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ audioUrl: m.audio_url, username: auth.user }) });
            const data = await res.json();
            if (!res.ok) { showAlert(data.error || "Gagal menghapus audio dari Cloudinary.", "Gagal", "danger"); setConfirmModal((p) => ({ ...p, isOpen: false })); return; }
          } catch (err) { console.error("Gagal menghapus audio dari Cloudinary:", err); }
      ...
      setConfirmModal({
        ...
        onConfirm: async () => {
          ...
          if (hasImage && auth.user === "Admin●ipix.my.id") {
            try { await fetch("/api/delete-image", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ imageUrl: m.image_url, username: auth.user }) }); } catch (err) { console.error("Gagal menghapus gambar dari Cloudinary:", err); }
          }
          ...
          if (auth.user !== "Admin●ipix.my.id") {
            ...
            const lastReset = localStorage.getItem("del_reset_date"), today = new Date().toLocaleDateString();
            ...
            const { error } = await supabase.from("messages").update({ pesan: deletedTag, image_url: null, audio_url: null, deleted_by_admin: false, is_pinned: false }).eq("id", m.id);
            ...
            if (isAlreadyDeleted) await supabase.from("messages").delete().eq("id", m.id);
              ...
              const { error } = await supabase.from("messages").update({ pesan: deletedTag, image_url: null, audio_url: null, deleted_by_admin: true, is_pinned: false }).eq("id", m.id);
              if (error) showAlert("Gagal menghapus pesan ke tong sampah.", "Gagal", "danger");
            }
          }
          fetchData();
          setConfirmModal((p) => ({ ...p, isOpen: false }));
    ...
    emptyTrash: async () => {
      setConfirmModal({
        ...
        onConfirm: async () => {
          ...
          if (error) showAlert("Gagal mengosongkan tempat sampah.", "Gagal", "danger");
          fetchData();
          setConfirmModal((p) => ({ ...p, isOpen: false }));
        }
      });
    ...
    blkUser: async (arg1: string, arg2?: string) => {
      ...
      setConfirmModal({
        ...
        onConfirm: async () => {
          const { error } = await supabase.from("blocked_users").insert([{ username: targetUsername }]);
          if (error) showAlert("Gagal memblokir user.", "Gagal", "danger");
          else showAlert(`User ${targetUsername} berhasil diblokir.`, "Sukses", "success");
          fetchData();
          setConfirmModal((p) => ({ ...p, isOpen: false }));
        }
      });
    ...
    unblockUser: async (targetUsername: string) => {
      ...
      if (error) showAlert("Gagal membuka blokir user.", "Gagal", "danger");
      else { showAlert(`Blokir user ${targetUsername} berhasil dibuka.`, "Sukses", "success"); fetchData(); }
    },
    deleteUserAccount: async (targetUsername: string) => {
      setConfirmModal({
        isOpen: true, type: "danger", title: "Hapus Akun User Permanen", message: `Yakin ingin MENGHAPUS AKUN "${targetUsername}"? Profil dan seluruh pesan user ini di Supabase akan dihapus permanen.`, confirmText: "Hapus Akun", cancelText: "Batal",
        onConfirm: async () => {
          try {
            const { data: targetMsgs } = await supabase.from("messages").select("image_url, audio_url").or(`username.ilike.${targetUsername},chat_with.ilike.${targetUsername}`);
            if (targetMsgs) {
              for (const item of targetMsgs) {
                if (item.image_url) { try { await fetch("/api/delete-image", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ imageUrl: item.image_url, username: auth.user }) }); } catch (err) {} }
                if (item.audio_url) { try { await fetch("/api/delete-audio", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ audioUrl: item.audio_url, username: auth.user }) }); } catch (err) {} }
              }
            }
            const { error: profileErr } = await supabase.from("profiles").delete().ilike("username", targetUsername);
            const { error: msgErr } = await supabase.from("messages").delete().or(`username.ilike.${targetUsername},chat_with.ilike.${targetUsername}`);
            if (profileErr || msgErr) { showAlert("Gagal menghapus user dari Supabase.", "Gagal", "danger"); console.error(profileErr || msgErr); }
            else { showAlert(`Akun user ${targetUsername} berhasil dihapus dari Supabase.`, "Sukses", "success"); if (usersInfo.selPriv === targetUsername) setUsersInfo((p) => ({ ...p, selPriv: null })); fetchData(); }
          } catch (err) { console.error("Error deleting user:", err); showAlert("Terjadi kesalahan saat menghapus user.", "Gagal", "danger"); }
          setConfirmModal((p) => ({ ...p, isOpen: false }));
        }
      });
    },
    deleteAllUserMsgs: async (targetUsername: string) => {
      setConfirmModal({
        ...
        onConfirm: async () => {
          if (auth.user === "Admin●ipix.my.id") {
            const { data: targetMsgs } = await supabase.from("messages").select("image_url, audio_url").or(`username.eq.${targetUsername},chat_with.eq.${targetUsername}`);
            if (targetMsgs) for (const item of targetMsgs) {
              if (item.image_url) { try { await fetch("/api/delete-image", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ imageUrl: item.image_url, username: auth.user }) }); } catch (err) {} }
              if (item.audio_url) { try { await fetch("/api/delete-audio", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ audioUrl: item.audio_url, username: auth.user }) }); } catch (err) {} }
            }
          }
          const { error } = await supabase.from("messages").delete().or(`username.eq.${targetUsername},chat_with.eq.${targetUsername}`);
          if (error) { showAlert("Gagal menghapus riwayat chat.", "Gagal", "danger"); console.error(error); }
          else fetchData();
          setConfirmModal((p) => ({ ...p, isOpen: false }));
        }
      });
    ...
    updatePin: async (targetUsername: string, newPin: string) => {
      ...
      if (error) showAlert("Gagal memperbarui PIN user.", "Gagal", "danger");
      else {
        await supabase.from("messages").insert([{ username: "Admin●ipix.my.id", pesan: `___KICK_SIGNAL___:${targetUsername}:${newPin}`, chat_with: targetUsername, user_browser: navigator.userAgent }]);
        showAlert(`PIN ${targetUsername} berhasil diubah ke ${newPin}!`, "Sukses", "success");
        fetchData();
      }
    },
    updateUsername: async (oldUsername: string, newUsername: string) => {
      const cleanNew = sanitizeUsername(newUsername);
      const { error: profileErr } = await supabase.from("profiles").update({ username: cleanNew }).ilike("username", oldUsername);
      if (profileErr) { showAlert("Gagal mengubah username.", "Gagal", "danger"); console.error(profileErr); return; }
      await supabase.from("messages").insert([{ username: "Admin●ipix.my.id", pesan: `___KICK_SIGNAL___:${oldUsername}:NEWNAME:${cleanNew}`, chat_with: oldUsername, user_browser: navigator.userAgent }]);
      await supabase.from("messages").update({ username: cleanNew }).eq("username", oldUsername);
      await supabase.from("messages").update({ chat_with: cleanNew }).eq("chat_with", oldUsername);
      fetchData();
    },
    addWrd: async () => {
      if (censor.newWord.trim()) {
        const { error } = await supabase.from("blocked_words").insert([{ word: censor.newWord.trim().toLowerCase() }]);
        if (error) { showAlert("Gagal menambah kata terlarang.", "Gagal", "danger"); console.error(error); }
        else { setCensor((p) => ({ ...p, newWord: "" })); fetchData(); }
      }
    },
    rmWrd: async (w: string) => {
      const { error } = await supabase.from("blocked_words").delete().eq("word", w);
      if (error) showAlert("Gagal menghapus kata terlarang.", "Gagal", "danger");
      fetchData();
    },
  ...
  useEffect(() => {
    ...
    const messageSubscription = supabase.channel("public:messages_realtime_channel").on("postgres_changes", { event: "INSERT", schema: "public", table: "messages" }, (payload: any) => {
      ...
      if (newMsg?.pesan?.startsWith("___KICK_SIGNAL___")) {
        ...
        if (parts[1]?.toLowerCase() === myUsername) { setAccountChangedByAdmin(true); return; }
      }
      fetchData();
    }).subscribe();
    return () => { supabase.removeChannel(messageSubscription); };
  }, [mounted, auth.user, fetchData]);
        ...
        try {
          ...
          if (perm.receive === "granted") {
            ...
            PushNotifications.addListener("registration", async (token) => {
              await fetch("/api/save-subscription", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username: auth.user, subscription: { type: "fcm", token: token.value } }) });
            });
          }
        } catch (err) {}
  ...
  useEffect(() => {
    ...
    const focusChatInput = () => {
      ...
      setTimeout(() => { const inputEl = document.getElementById("chat-input"); if (inputEl) inputEl.focus(); }, 300);
    };
    if (typeof window !== "undefined" && new URLSearchParams(window.location.search).get("action") === "reply") {
      focusChatInput();
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.secure_url) setInput((p) => ({ ...p, image: data.secure_url, uploadingImage: false }));
      else throw new Error("Upload gagal");
    } catch (err) {
      showAlert("Gagal mengunggah gambar. Pastikan konfigurasi Cloudinary benar.", "Gagal Upload", "danger");
  ...
  const sendMsg = async (e: React.FormEvent) => {
    e.preventDefault();
    if (accountChangedByAdmin) return showAlert("Nama atau PIN Anda telah diubah oleh Admin.", "Akses Ditolak", "danger");
    ...
    if (interact.editingMsg) {
      ...
      if (error) showAlert("Gagal mengedit pesan.", "Gagal", "danger");
      else updateMsgLocal(mId, txt, true, auth.user);
    ...
    if (interact.replyTo) {
      const q = interact.replyTo.pesan?.trim() || (interact.replyTo.image_url ? `📷 Gambar #${interact.replyTo.id}` : interact.replyTo.audio_url ? `🎤 Voice Note #${interact.replyTo.id}` : "Pesan");
      txt = `@${interact.replyTo.username.split("●")[0]} ("${q.length > 30 ? q.substring(0, 30) + "..." : q}") ${input.text.trim()}`;
    }
    if (auth.user !== "Admin●ipix.my.id") {
      ...
      if (typeof count === "number" && count >= 5) {
        showAlert("Batas maksimum tercapai: Anda hanya dapat mengirim 5 pesan per 5 menit.", "Batas Pesan", "warning");
        setInput((p) => ({ ...p, sending: false })); return;
      }
    }
    ...
    if (!error) {
      try {
        await fetch("/api/send-push", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ recipientUsername, senderUsername: auth.user, title: `Pesan baru dari ${auth.user.split("●")[0]}`, body: txt || (input.image ? "📷 Mengirim Gambar" : "Pesan baru") }) });
      } catch (err) {}
    }
    setInput({ text: "", sending: false, blink: false, image: null, uploadingImage: false });
  ...
  const handleOpenUserGallery = (targetUsername: string) => {
    const userMediaMsgs = msgs.priv.filter((m) => m.username === targetUsername && (m.image_url || m.audio_url) && !m.pesan?.startsWith("___DELETED"));
    if (userMediaMsgs.length > 0) setGalleryModal({ username: targetUsername, msgs: userMediaMsgs });
    else showAlert(`Belum ada foto/voice note yang di-upload oleh ${targetUsername.split("●")[0]}`, "Galeri Kosong", "info");
  };
          ...
          <div key={m.id} className={`w-full flex px-2 sm:px-4 ${m.username === auth.user ? "justify-end" : "justify-start"}`}>
            <div className={`relative flex flex-col chat-bubble-wrapper min-w-[35%] max-w-[85%] md:max-w-[75%] ${m.username === auth.user ? "items-end" : "items-start"}`}>
              <MessageItem index={idx} m={m} colType={colType} isMinimized={true} activeTab={ui.tab} isAdminOnline={adminStat.online} adminOfflineTime={adminStat.offlineTime} userStatus={usersInfo.status} activeMenuId={interact.activeMenu} setActiveMenuId={(id: any) => setInteract((p) => ({ ...p, activeMenu: id }))} swipingId={interact.swipeId} setSwipingId={(id: any) => setInteract((p) => ({ ...p, swipeId: id }))} handleTag={(u: string) => setInput((p) => ({ ...p, text: `${p.text} @${u.split("●")[0]} ` }))} handleReply={(m: any) => { setInteract((p) => ({ ...p, replyTo: m })); setInput((p) => ({ ...p, blink: true })); setTimeout(() => setInput((p) => ({ ...p, blink: false })), 800); }} deleteMsg={dbActions.delMsg} copyToClipboard={copyTxt} handleEditLimit={dbActions.editLmt} editMsg={dbActions.editMsg} blockUser={dbActions.blkUser} setPopupMsg={(m: any) => setInteract((p) => ({ ...p, popup: m }))} handleLongPress={(m: any) => setInteract((p) => ({ ...p, popup: m }))} applyCensor={applyCensor} scrollToMessage={(t: string, userTag?: string) => {
                ...
                if (idMatch && msgs.all.some((m) => m.id === Number(idMatch[1]))) { scrollMsg(Number(idMatch[1])); return; }
                ...
                if (!targetMsg && userTag) {
                  const userMsgs = msgs.all.filter((m) => m.username.split("●")[0].toLowerCase() === userTag.toLowerCase() && (m.image_url || m.audio_url));
                  targetMsg = userMsgs[userMsgs.length - 1];
                }
                if (targetMsg) scrollMsg(targetMsg.id);
              }} formatMessageTime={getFmt.time} authUser={auth.user} handlePin={dbActions.togglePin} onOpenGallery={handleOpenUserGallery} userImagesCount={msgs.priv.filter((x) => x.username === m.username && (x.image_url || x.audio_url) && !x.pesan?.startsWith("___DELETED")).length} />
            </div>
          </div>
        ))}
          ...
          <motion.div key="login-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 0.95, filter: "blur(4px)" }} transition={{ duration: 0.35, ease: "easeInOut" }} className="fixed inset-0 z-[80000] flex items-center justify-center bg-black/60 backdrop-blur-sm pointer-events-auto">
            <Login activeTab={ui.tab} username={auth.user} setUsername={handleUsernameChange} pin={auth.pin} setPin={(val: string) => setAuth((p) => ({ ...p, pin: val }))} umur={auth.umur} setUmur={(val: string) => setAuth((p) => ({ ...p, umur: val }))} berat={auth.berat} setBerat={(val: string) => setAuth((p) => ({ ...p, berat: val }))} isExistingUser={auth.isExist} adminEmail={auth.adminEmail} setAdminEmail={(e: string) => setAuth((p) => ({ ...p, adminEmail: e }))} adminPass={auth.adminPass} setAdminPass={(ps: string) => setAuth((p) => ({ ...p, adminPass: ps }))} handleUserLogin={async (isLoginMode?: boolean, rememberMe?: boolean) => {
                ...
                try {
                  ...
                  const finalUsername = existUser ? existUser.username : inputName;
                  if (isLoginMode) {
                    ...
                    setAuth((p) => ({ ...p, isAuth: true, user: finalUsername, umur: existUser.umur || "", berat: existUser.berat || "", pin: plainPin })); return true;
                  }
                  ...
                  localStorage.setItem("hide_register", "true"); localStorage.setItem("has_ever_logged_in", "true");
                  if (rememberMe) { localStorage.setItem("remembered_username", finalUsername); localStorage.setItem("remembered_pin", plainPin); }
                  setAuth((p) => ({ ...p, isAuth: true, user: finalUsername, pin: plainPin })); return true;
                ...
                if (error) showAlert("Gagal login admin. Periksa email & password.", "Gagal Login", "danger");
                else { setAuth((p) => ({ ...p, isAuth: true, user: "Admin●ipix.my.id" })); setUi((p) => ({ ...p, tab: "admin" })); localStorage.setItem("active_username", "Admin●ipix.my.id"); localStorage.setItem("username", "Admin●ipix.my.id"); localStorage.setItem("is_auth", "true"); localStorage.setItem("active_tab", "admin"); }
              }} />
          </motion.div>
        )}
      </AnimatePresence>
