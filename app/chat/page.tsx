...
export default function Home() {
  ...
  const handleOpenUserGallery = (targetUsername: string) => { const userImagesMsgs = msgs.priv.filter((m) => m.username === targetUsername && m.image_url && !m.pesan?.startsWith("___DELETED")); if (userImagesMsgs.length > 0) setGalleryModal({ username: targetUsername, msgs: userImagesMsgs }); else showAlert(`Belum ada foto yang di-upload oleh ${targetUsername.split("●")[0]}`, "Galeri Kosong", "info"); };
