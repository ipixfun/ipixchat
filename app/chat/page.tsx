...
export default function Home() {
  ...
  const triggerAdminChangeNotice = useCallback((newUsername?: string) => {
    setAdminNoticeModal((prev) => {
      ...
      return { isOpen: true, message: "Nama atau PIN Anda telah diubah oleh Admin. Silakan klik tombol di bawah untuk kembali ke halaman Login.", newUsername };
    });
  }, []);
  ...
  const handleConfirmAdminChangeLogout = async () => {
    ...
    await supabase.auth.signOut();
    setAuth({ isAuth: false, isExist: true, user: newUsername || auth.user || "", adminEmail: "", adminPass: "", pin: "", umur: "", berat: "" });
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      if (auth.isAuth && targetUser !== "Admin●ipix.my.id") {
        ...
        if (profileCheck) {
          if (savedPin && profileCheck.pin !== savedPin) { triggerAdminChangeNotice(profileCheck.username); return; }
        } else { triggerAdminChangeNotice(); return; }
      }
      const isAdminTab = ui.tab === "admin";
      let queryFilter = "";
      if (isAdminTab) {
        const selUser = usersInfo.selPriv;
        queryFilter = selUser ? `username.ilike.${selUser},chat_with.ilike.${selUser}` : "id.gt.0";
      } else {
      ...
      if (isAdminTab && !usersInfo.selPriv) {
        ...
        if (aP) {
          const uMap = new Map(); const c: Record<string, number> = {}; const userMsgTotal: Record<string, number> = {}; const adminMsgTotal: Record<string, number> = {};
          ...
          aP.forEach((m) => { 
            ...
            } else if (m.chat_with) { adminMsgTotal[m.chat_with] = (adminMsgTotal[m.chat_with] || 0) + 1; }
          });
          aP.forEach((m: any) => { 
            if (!m.username?.toLowerCase().includes("admin") && !uMap.has(m.username)) { 
              const userProfile = profileMap.get(m.username.toLowerCase());
              uMap.set(m.username, { ...m, last_active: m.created_at, count: isAdminTab ? 0 : (c[m.username] || 0), pin: userProfile?.pin || "", umur: userProfile?.umur || "", berat: userProfile?.berat || "", totalUserMsgs: userMsgTotal[m.username] || 0, totalAdminMsgs: adminMsgTotal[m.username] || 0, last_message: m.pesan }); 
            } 
          });
  ...
  const dbActions = {
    ...
    updatePin: async (targetUsername: string, newPin: string) => {
      ...
      if (error) { alert("Gagal memperbarui PIN user."); } else { 
        ...
        alert(`PIN ${targetUsername} berhasil diubah ke ${newPin}!`); fetchData(); 
      }
    },
    ...
    togglePin: async (msg: any) => {
      try {
        const nextState = !msg.is_pinned; const isMsgFromAdmin = msg.username === "Admin●ipix.my.id";
        if (nextState) {
    ...
    editPinned: async (currentPinned: any) => {
      ...
      const defaultText = "halo semua"; const currentText = currentPinned ? currentPinned.pesan : defaultText;
      setPromptModal({
        isOpen: true, title: "Edit Pesan Sematan:", defaultValue: currentText,
  ...
  useEffect(() => {
    ...
    const messageSubscription = supabase.channel("public:messages_realtime_channel").on("postgres_changes", { event: "INSERT", schema: "public", table: "messages" }, (payload: any) => {
      ...
      if (newMsg && newMsg.pesan && newMsg.pesan.startsWith("___KICK_SIGNAL___")) {
        ...
        const parts = newMsg.pesan.split(":"); const targetUser = parts[1]?.toLowerCase();
        if (targetUser === myUsername) {
          const newName = parts[2] === "NEWNAME" ? parts[3] : undefined;
          triggerAdminChangeNotice(newName); return;
        }
      }
      fetchData();
    }).subscribe();
  ...
  const sendMsg = async (e: React.FormEvent) => {
    ...
    if (interact.editingMsg) {
      const mId = interact.editingMsg.id; const isUserAdmin = auth.user === "Admin●ipix.my.id";
      if (!isUserAdmin) {
    ...
    if (interact.replyTo) { 
      const qText = interact.replyTo.pesan?.trim(); const hasImg = !!interact.replyTo.image_url;
            ...
            <div className={`relative flex flex-col chat-bubble-wrapper min-w-[35%] ${maxWidthClass} ${isMine ? "items-end" : "items-start"}`}>
              <MessageItem 
                index={idx} m={m} colType={colType} isMinimized={true} activeTab={ui.tab} isAdminOnline={adminStat.online} adminOfflineTime={adminStat.offlineTime} userStatus={usersInfo.status} activeMenuId={interact.activeMenu} setActiveMenuId={(id: any) => setInteract((p) => ({ ...p, activeMenu: id }))} swipingId={interact.swipeId} setSwipingId={(id: any) => setInteract((p) => ({ ...p, swipeId: id }))} handleTag={(u: string) => setInput((p) => ({ ...p, text: `${p.text} @${u.split("●")[0]} ` }))} handleReply={(m: any) => { setInteract((p) => ({ ...p, replyTo: m })); setInput((p) => ({ ...p, blink: true })); setTimeout(() => setInput((p) => ({ ...p, blink: false })), 800); }} deleteMsg={dbActions.delMsg} copyToClipboard={copyTxt} handleEditLimit={dbActions.editLmt} editMsg={dbActions.editMsg} blockUser={dbActions.blkUser} setPopupMsg={(m: any) => setInteract((p) => ({ ...p, popup: m }))} handleLongPress={(m: any) => setInteract((p) => ({ ...p, popup: m }))} applyCensor={applyCensor} scrollToMessage={(t: string, userTag?: string) => { 
                  ...
                  if (idMatch) {
                    const targetId = Number(idMatch[1]); const exist = msgs.all.some((m) => m.id === targetId);
      ...
      {adminNoticeModal.isOpen && (
        <div className="fixed inset-0 z-[999999] bg-black/90 backdrop-blur-md flex items-center justify-center p-4 animate-fadeIn select-none">
          <div className="w-full max-w-sm bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl flex flex-col items-center text-center gap-4 animate-scaleUp">
            ...
            <div className="flex flex-col gap-1.5">
              <h3 className="text-base font-bold text-white tracking-wide">Pemberitahuan Akun</h3>
              <p className="text-xs text-slate-300 font-medium leading-relaxed">{adminNoticeModal.message}</p>
            </div>
            <button type="button" onClick={handleConfirmAdminChangeLogout} className="w-full mt-2 py-3 px-4 bg-amber-500 hover:bg-amber-400 text-slate-950 font-extrabold text-xs rounded-xl shadow-lg shadow-amber-500/20 transition-all active:scale-95 border border-amber-400/50 cursor-pointer">
              Ke Halaman Login ➔
            </button>
          </div>
        </div>
