...
export default function Head({ auth, ui, adminStat, onlineUsers, currentHash, getFmt, handleLogout }: HeadProps) {
    ...
    <>
      ...
      <div className="sticky top-0 z-20 flex flex-col backdrop-blur-xl border-b transition-colors duration-500 overflow-hidden shadow-sm" style={{ backgroundColor: "color-mix(in srgb, var(--background) 80%, transparent)", borderColor: "var(--card-border)" }}>
        ...
        <div className="p-4 relative z-10">
          ...
          {auth.isAuth && (
            <button onClick={handleLogout} className="absolute top-4 right-4 text-[10px] bg-red-500/90 hover:bg-red-600 text-white px-4 py-1.5 rounded-full shadow-lg backdrop-blur-sm z-30 transition-all duration-300 font-semibold active:scale-95 ring-1 ring-red-400/50 hover:shadow-red-500/30">
              Keluar
            </button>
          )}
          ...
          <div className="flex justify-between items-center relative z-20">
            <div className="flex flex-col max-w-[65%] gap-0.5">
              <span className="text-[10px] uppercase tracking-widest font-semibold opacity-70" style={{ color: "var(--foreground)" }}>
                {getFmt.greet()}
              </span>
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-sm font-black tracking-tight" style={{ color: "var(--foreground-heading)" }}>
                  {auth.user || "Guest"}
                </span>
        ...
        {auth.isAuth && ui.tab === "admin" && (
          <div className="relative z-10 flex items-center overflow-x-auto gap-2.5 px-4 py-2.5 border-t text-xs whitespace-nowrap [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] transition-colors duration-500 shadow-inner" style={{ backgroundColor: "color-mix(in srgb, var(--card-bg) 60%, transparent)", borderColor: "var(--card-border)", backdropFilter: "blur(12px)" }}>
            ...
            {onlineUsers.map((u) => (
              <span key={u} className="px-3 py-0.5 rounded-full text-[10px] font-medium border shadow-sm transition-all hover:scale-105 cursor-default" style={{ backgroundColor: "color-mix(in srgb, var(--background) 50%, transparent)", color: "var(--foreground)", borderColor: "var(--card-border)" }}>
                {u.split("●")[0]}
              </span>
            ))}
